...
description: A Step-by-step Guide to Using Agniflow UI
---

## Quickstart

Running tight on schedule? No worries! Check out our quickstart examples to get started with Agniflow UI in seconds.
...
The next package you'll need is `@agniflow-ui/panda-preset`. It extends the Panda Base Preset with advanced color options and opinionated design tokens.

```bash
npm install @agniflow-ui/panda-preset -D
```
...
Use the Agniflow UI CLI to add your first component. For example, to add a button, run:

```bash
npx @agniflow-ui/cli components add button
```
...
```bash
npx @agniflow-ui/cli components add avatar button card
```

Or use the `--all` flag to install all available components:

```bash
npx @agniflow-ui/cli components add --all
```
