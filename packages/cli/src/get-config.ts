...
export const getConfig = (): Effect.Effect<Config, never, never> =>
  pipe(
    ...
    Effect.flatMap((packageDirectory) =>
      pipe(
        Effect.succeed(path.join(packageDirectory, 'agniflow-ui.json')),
        Effect.flatMap((configFilePath) =>
          pipe(
            ...
            Effect.catchAll(() => {
              p.note(
                'No agniflow-ui.json found or it is outdated in the project. Creating new config file.',
                'Info',
              )
              return pipe(
                Effect.promise(promptConfig),
                Effect.map(({ framework, components, recipes }) => ({
                  $schema: 'https://agniflow-ui.com/registry/latest/schema.json',
                  framework,
                  paths: {
                    components,
                    recipes,
